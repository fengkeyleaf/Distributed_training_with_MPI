...
// CIFAR10 dataset description can be found at https://www.cs.toronto.edu/~kriz/cifar.html.
    constexpr uint32_t kTrainSize = 10000;
    ...
    const std::vector<std::string> kTrainDataBatchFilesRoot = {
            "data_batch_1.bin"
    };

    const std::vector<std::string> kTrainDataBatchFilesWorker1 = {
            "data_batch_2.bin"
    };
...
// Partially based on https://github.com/pytorch/pytorch/blob/master/torch/csrc/api/src/data/datasets/mnist.cpp.
    std::pair<torch::Tensor, torch::Tensor> read_data( const std::string &root, bool train, int world_rank ) {
        const auto &files = train ? ( world_rank == 0 ? kTrainDataBatchFilesRoot : kTrainDataBatchFilesWorker1 ) : kTestDataBatchFiles;
...
CIFAR10::CIFAR10( const std::string &root, int world_rank, Mode mode ) : mode_( mode ) {
    auto data = read_data( root, mode == Mode::kTrain, world_rank );
